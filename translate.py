...
#working on code
    ...
    reverse_comp_seq = reverse_and_complement(rna_sequence)
    reverse_comp_pep = get_all_translations(rna_sequence = reverse_comp_seq, genetic_code = genetic_code)
    pep += reverse_comp_pep
    if not pep:
        return "" 
    if len(pep) < 2:
        return pep[0]
